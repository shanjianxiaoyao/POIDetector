...
public:
    ...
    virtual bool setImage(const Mat &image, Size _origWinSize, Mat* sum_scale, Mat* sqsum_scale, Mat* tilted_scale );
...
protected:
    ...
    int offset;
    double varianceNormFactor;//haar的归一因子
};
...
public:
    ...
    virtual bool setImage( const Mat& image, Size winSize, vector<Mat>& hist, Mat& norm );
    virtual bool setWindow( Point pt );
    double operator()(int featureIdx) const
    {
        return featuresPtr[featureIdx].calc(offset);
    ...
    void integralHistogram( const Mat& srcImage, vector<Mat> &histogram, Mat &norm, int nbins );
private:
    Size origWinSize;
    Ptr<vector<Feature> > features;
    Feature* featuresPtr;
...
/* 有两个Evaluator的预测函数 */
//template<typename T>
inline int predictOrderedStump( CascadeClassifier& cascade, Ptr<FeatureEvaluator> &_featureEvaluator_haar, Ptr<FeatureEvaluator> &_featureEvaluator_hog, double& sum )
{
	int nodeOfs = 0, leafOfs = 0;
	//HaarEvaluator& featureEvaluator_haar =  (HaarEvaluator&)*_featureEvaluator_haar;
	HOGEvaluator& featureEvaluator_hog = (HOGEvaluator&)*_featureEvaluator_hog;
	...
	for( int stageIdx = 0; stageIdx < nstages; stageIdx++ )
	{
		...
		for( int i = 0; i < ntrees; i++, nodeOfs++, leafOfs+= 2 )
		{
			...
			double value = 0;
			//if ( "HAAR" == stage.featureType )
			//{
			//	value = featureEvaluator_haar(node.featureIdx);
			//}
			//else if( "HOG" == stage.featureType )
			//{
				value = featureEvaluator_hog(node.featureIdx);
			//}
			sum += cascadeLeaves[ value < node.threshold ? leafOfs : leafOfs + 1 ];
		}
